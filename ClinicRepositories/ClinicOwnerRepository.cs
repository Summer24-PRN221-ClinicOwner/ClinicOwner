    ...
    public class ClinicOwnerRepository : GenericRepository<ClinicOwner>, IClinicOwnerRepository
    {
        ...
        public List<ClinicReportDataObject> GetClinicReport(DateTime startDate, DateTime endDate)
        {
            ...
            while (startDate.Date <= endDate.Date)
            {
                ...
                var dailyAppointments = _context.Appointments.Include(item => item.Service).Include(item => item.Dentist).Where(item => item.AppointDate.Date == startDate.Date && (item.Status == (int)AppointmentStatus.Reported || item.Status == (int)AppointmentStatus.Absent)).ToList();

                // Check if there are any appointments for this date
                if (dailyAppointments.Count != 0)
                {
                    var dentistAppointmentGroups = _context.Dentists.Include(item => item.Appointments).Select(item => new { Key = item, Value = item.Appointments.Where(item => item.AppointDate.Date == startDate.Date && (item.Status == (int)AppointmentStatus.Reported || item.Status == (int)AppointmentStatus.Absent)).Count() });
                    var serviceAppointmentGroups = _context.Services.Include(item => item.Appointments).Select(item => new { Key = item, Value = item.Appointments.Where(item => item.AppointDate.Date == startDate.Date && (item.Status == (int)AppointmentStatus.Reported || item.Status == (int)AppointmentStatus.Absent)).Count() });
        ...
        {
            var dailyAppointments = _context.Appointments.Include(item => item.Service).Include(item => item.Dentist).Where(item => item.AppointDate.Date >= startDate.Date && item.AppointDate.Date <= endDate.Date && (item.Status == (int)AppointmentStatus.Reported || item.Status == (int)AppointmentStatus.Absent)).ToList();

            // Check if there are any appointments for this date
            var dentistAppointmentGroups = _context.Dentists.Include(item => item.Appointments).Select(item => new { Key = item, Value = item.Appointments.Where(item => item.AppointDate.Date >= startDate.Date && item.AppointDate.Date <= endDate.Date && (item.Status == (int)AppointmentStatus.Reported || item.Status == (int)AppointmentStatus.Absent)).Count() });
            var serviceAppointmentGroups = _context.Services.Include(item => item.Appointments).Select(item => new { Key = item, Value = item.Appointments.Where(item => item.AppointDate.Date >= startDate.Date && item.AppointDate.Date <= endDate.Date && (item.Status == (int)AppointmentStatus.Reported || item.Status == (int)AppointmentStatus.Absent)).Count() });
